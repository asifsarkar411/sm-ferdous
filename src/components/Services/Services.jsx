      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
        {services.map(service => (
          ...
          >
            ...
            <h3 style={{ marginBottom: '0.75rem', fontSize: '1.25rem', fontWeight: '700', color: 'var(--color-text-primary)' }}>{service.title}</h3>
            <p style={{ color: 'var(--color-text-secondary)', lineHeight: 1.65, fontSize: '0.95rem' }}>{service.description}</p>
          </div>
        ))}
      </div>
