...
export default async function Services({ services: propServices }) {
  const services = propServices !== undefined ? propServices : await prisma.service.findMany();
    ...
    <section id="services" className="section container">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem' }}>
        {services.map(service => (
          <div key={service.id} style={{ padding: '2rem', backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: '16px', transition: 'transform 0.2s ease', willChange: 'transform' }}>
